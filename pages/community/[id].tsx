  ...
  return (
    <Layout canGoBack>
      <div>
        ...
        {/* Profile */}
        <div className="flex px-4 py-3 border-b items-center space-x-3">
        ...
        <div className=" px-4 py-2">
          <span className="mt-2 text-gray-700 ">
            <span className="text-orange-500 font-semibold px-1">Q.</span>
            {data?.post?.question}
          </span>
          ...
          {data?.post?.answer?.map((ans) => (
            <div key={ans.id} className="flex items-start space-x-3">
              ...
              <div>
                ...
                <p className="text-gray-700 mt-2">{ans.answer}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <form onSubmit={handleSubmit(onValid)} className="px-4">
          ...
          <button className="mt-2 w-full bg-orange-500 hover:bg-orange-600 text-white py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-semibold focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 focus:outline-none ">
            {answerLoading ? "Loading..." : "Upload product"}
          </button>
        </form>
      </div>
    </Layout>
